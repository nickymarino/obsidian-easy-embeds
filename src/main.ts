...
export default class TwitterEmbedPlugin extends Plugin {
	...
	async onload() {
		...
		this.registerEditorExtension(
			buildEmbedderExtension(liveEditorEmbedders)
		);
			...
			if (state == "preview" || state == "live") {
				...
				this.app.workspace.onLayoutReady(() => {
					if (
						...
						console.log(
							"Twitter blockquote(s) found, refreshing..."
						);
		...
		this.registerEvent(
			this.app.workspace.on(
				"active-leaf-change",
				refreshTwitterIfActiveBlockquotes
			)
		);
		...
		this.registerMarkdownPostProcessor(
			(el: HTMLElement, ctx: MarkdownPostProcessorContext) => {
				const uiTheme: UITheme = document.body.classList.contains(
					"theme-dark"
				)
					? "dark"
					: "light";

				el.querySelectorAll("a.external-link").forEach(
					(anchor: HTMLAnchorElement) => {
						...
						for (const embedder of liveEditorEmbedders) {
							if (embedder.canAddEmbed(link)) {
								...
								embedContainer.setAttribute(
									"class",
									"embed-container"
								);
								anchor.parentElement.parentElement.insertBefore(
									embedContainer,
		...
		this.registerMarkdownCodeBlockProcessor(
			...
			(
				...
				const uiTheme: UITheme = document.body.classList.contains(
					...
					? "dark"
					: "light";
				const result = this.twitter.parseCodeBlock(
					source,
					this.settings,
					uiTheme
				);
