...
export default class DropboxEmbedder implements Embedder {
	...
	addEmbed(parent: HTMLElement, url: string) {
		...
		const isImageRegex = new RegExp(
			/https?:\/\/(www\.)?dropbox.com\/(?<slug>s\/[\w\d]+\/\S+\.(png|jpeg|gif|jpg))(\?dl=0)?/,
			"i"
		);
	...
	loadDropboxJS(force = false) {
		const existingDropbox = document.getElementById(
			this.dropboxJSIdentifier
		);
