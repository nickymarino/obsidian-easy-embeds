...
export default class TwitterEmbedder implements Embedder {
	...
	addEmbed(
		...
		if (!status) {
			console.log(
				`Error: No status could be parsed from Twitter URL: ${url}`
			);
			return;
		}
		...
		window.twttr.ready(() => {
			window.twttr.widgets.createTweet(status, parent, options);
		});
	}
	...
	parseCodeBlock(
		...
		const finalOptions = this.overrideOptions(
			defaults,
			optionsFromCodeBlock,
			uiTheme
		);
	...
	parseStatusIDFromUrl(url: string): string | undefined {
		...
		const tweetRegex =
			/https?:\/\/(?:mobile\.)?twitter\.com\/(?:#!\/)?(\w+)\/status(es)?\/(?<status_id>\d+)/;
		return url.trim().match(tweetRegex)?.groups?.status_id;
	}

	overrideOptions(
		defaults: Settings,
		overrides: CodeBlockOptions,
		uiTheme: UITheme
	): CodeBlockOptions {
